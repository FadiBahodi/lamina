...
\midrule
Source identity & Hashes, ordered text units, locators, adjacent extraction context & Working for supported text; not visual understanding \\
...
The generic method runtime executes data-only JSON nodes with hard dependencies, task-field selection, worker and lane capacities, cache receipts, and operator-selected observations. Its generic response check requires a JSON object; declared expected shapes are hints, not fully enforced semantic contracts. The source-aware production engine reads selected teaching sources in bounded, owned windows with full neighboring units, asks a model for a natural section route and a representation per section, then writes and reviews sections under separate concurrency limits. One material review finding can trigger a section-local repair and re-review; unresolved findings remain visible. A named revision changes the selected section request and reuses unchanged cached work. The local project API persists state, marks unfinished work interrupted after restart, and can resume from retained plans and cache. The CLI and local Projects interface call the same engine. Export writes Markdown, HTML, a report, plan and optional PDF; assessment export separates candidate and examiner files. These mechanisms are implemented and structurally tested. They do not automatically infer prose-section dependencies after a revision, interpret arbitrary figures, perform a blind candidate solve, or prove audio or clinical quality. The generic method graph remains a separate execution path rather than replacing the production engine.
