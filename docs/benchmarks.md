...
The older 25,000-word fixture also runs against the current engine: [raw result](../benchmarks/results/source-workflows-current.json). It makes 22 cold calls (one read, one route, ten writes and ten reviews), zero repeat calls and two calls for one section revision. Reader source exposure is 1.00×. This fixture declares only a byte limit, returns fixed short ideas, and imposes no real model output limit; its single reader call cannot establish a suitable textbook batch size.
