...
The public website supports editing and downloading a method, plus inspection of recorded examples. Execution requires the downloaded local app. Browser run status is currently held in memory; the underlying committed jobs and runtime receipts persist in SQLite. Failed-node diagnostics are redacted in browser responses and downloadable receipts; full diagnostics remain local. A successfully executed reviewer node can still return a domain verdict such as `revise`: runtime completion is not approval to publish.
