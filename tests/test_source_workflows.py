...
from lamina.providers import CommandProvider, StageProvider
...
def test_assigned_context_is_not_ownership_and_revision_is_local(tmp_path):
    ...
    assert {u["id"] for u in request["assigned_units"]} == {"u3", "u4"}
    assert {u["id"] for u in request["earlier_evidence_units"]} == {"u2"}
...
def test_local_source_edit_reuses_other_reader_interpretations(tmp_path):
    ...
    path.write_text(
        "# Wind\n\n## Blades\nWind turns the blades. Another statement.\n\n## Shaft\nThe shaft carries rotation. Another statement.\n"
    )
    ws, provider = Workspace(tmp_path / "db"), FixtureProvider()
    ingest_paths([path], ws)
    old_id = ws.sources()[0]["id"]
    first = plan_production(ws, provider, "Explain", [old_id], {"workflow": "planned"})
    path.write_text(path.read_text().replace("turns", "can turn"))
    ingest_paths([path], ws)
    new_id = next(s["id"] for s in ws.sources() if s["id"] != old_id)
    before = len(provider.requests)
    second = plan_production(ws, provider, "Explain", [new_id], {"workflow": "planned"})
