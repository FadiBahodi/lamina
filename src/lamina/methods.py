...
def validate_method(value: dict) -> dict:
    ...
    method = {
        "schema_version": "1",
        "family": _id(value["family"], "family"),
        "id": _id(value["id"], "id"),
    }
    if not isinstance(value["version"], str) or not _VERSION.fullmatch(
        value["version"]
    ):
    ...
    for key in ("contexts", "exclusions", "limits"):
        ...
        if (
            ...
            or len(items) > 20
            or (key == "contexts" and not items)
        ):
            raise ValueError(f"applicability.{key} must be a bounded list")
        method["applicability"][key] = [
            _text(x, f"applicability.{key}", 300) for x in items
        ]
    ...
    method["resources"] = {
        "workers": workers,
        "lanes": dict(sorted(clean_lanes.items())),
    }
    nodes = value["nodes"]
    ...
    for raw in nodes:
        ...
        if (
            ...
            or set(raw) - required - {"task_keys", "expected_shape", "observation_ids"}
        ):
            raise ValueError(
                "each node needs id, role, instructions, lane, depends_on, input; task_keys, expected_shape, and observation_ids are optional"
            )
        ...
        if task_keys is not None and (
            ...
            raise ValueError(
                f"{nid}.task_keys must be a unique list of top-level task keys"
            )
        ...
        if (
            ...
            or any(
                not isinstance(oid, str) or not _OBSERVATION_ID.fullmatch(oid)
                for oid in observation_ids
            )
            or len(observation_ids) != len(set(observation_ids))
        ):
            raise ValueError(
                f"{nid}.observation_ids must be unique local observation IDs (at most 20)"
            )
        clean_nodes.append(
            {
                "id": nid,
                "role": _text(raw["role"], f"{nid}.role", 160),
